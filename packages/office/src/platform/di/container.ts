// DI-контейнер. Перенесён из проекта diode (src/vs/platform/instantiation/common/diContainer.ts) как есть.
//
//   токен        `token<T>('Имя')` — типизированный ключ сервиса;
//   сервис       класс со `static dependencies = [ТокенА, ТокенБ] as const` и конструктором с теми же аргументами
//                (компилятор сверяет порядок и типы) — или просто фабрика `() => значение`;
//   модуль       функция, которая биндит набор связанных сервисов, возможно с типизированным контекстом;
//   профиль      набор модулей: production (modules/productionProfile.ts) и test (modules/testProfile.ts).
//
// Всё — синглтоны, разрешаются лениво: порядок bind не важен.
 ...
 * опциональным типизированным контекстом (каталог проекта, порт и т.п.).
 *
...
export class Container implements ServiceAccessor {
     ...
     *
     *     container
     *         .use(manifestModule, { manifest, prompts })
     *         .use(storageModule, { journalFile })
     *         .use(officeModule)
     */
    ...
    public get<T>(tok: Token<T>): T {
        ...
        if (!factory) {
            throw new Error(`No binding for "${tok.id}"`)
        }
